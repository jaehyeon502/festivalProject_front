...
export default function FreeBoardDetailView() {
  ...
  //          Error Handler          //
  const onPostFreeBoardCommentErrorHandler = (error: any) => console.log(error.message);
  const onDeleteFreeBoardErrorHandler = (error: any) => console.log(error.message);
  const getFreeBoardError = (error: any) => console.log(error.message);
  const onRecommendErrorHandler = (error: any) => console.log(error.message);
  ...
  return (
    <Box sx={{ backgroundColor: '#c0c0c0' }}>
      <Box sx={{ backgroundColor: '#FFFFFF', height: '80%', ml: '200px', mr: '200px' }}>
        ...
        <Menu sx={{ position: 'absolute', top: '-600px', left: '16px' }} anchorEl={anchorElement} open={menuOpen} onClose={onMenuCloseHandler}>
        ...
        <Box>
          <Box sx={{ ml: '60px', mr: '60px', mt: '30px', height:'200px' }}>
            ...
            {board?.boardImgUrl && (<Box sx={{ width: '50%', mt: '20px' }} component='img' src={board?.boardImgUrl} />)}
          </Box>
